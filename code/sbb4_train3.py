...
label_flag = 3
train_buy = jdata_data[(jdata_data['action_time']>='2018-03-26')
                       & (jdata_data['action_time']<'2018-04-02')
                       & (jdata_data['type']==2)][['user_id','cate','shop_id']].drop_duplicates()
train_buy['label'] = 1
# 候选集 时间 ： '2018-03-26'-'2018-04-08' 最近两周有行为的（用户，类目，店铺）
win_size = 4#如果选择两周行为则为2 三周则为3
train_set = jdata_data[(jdata_data['action_time']>='2018-02-26')
                       & (jdata_data['action_time']<'2018-03-26')][['user_id','cate','shop_id']].drop_duplicates()
...
test_set = jdata_data[(jdata_data['action_time'] >= '2018-03-19') & (jdata_data['action_time'] < '2018-04-16')][
